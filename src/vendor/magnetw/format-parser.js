...
function extractSizeText(str) {
  const match = new RegExp(
    `(\\d+(\\.\\d+)?)(&nbsp;| )*(${sizeUnitRegx}|${sizeUnitSpareRegx})`,
    "gi"
  ).exec(str);
    ...
    if (dateText) {
      ...
      if (/yesterday|昨天/.test(dateText)) {
        ...
        const unit = [
          ...
          { regx: "year|年", name: "years" },
          { regx: "month|月", name: "months" },
          { regx: "day|天", name: "days" },
          { regx: "hour|小时", name: "hour" },
          { regx: "minute|分钟", name: "minutes" },
          { regx: "second|秒", name: "seconds" }
        ];
        for (let i = 0; i < unit.length; i++) {
          const dateTextMatches = new RegExp(
            `\\d+( {0,3})${unit[i].regx}`,
            "gi"
          ).exec(dateText);
          if (dateTextMatches) {
  ...
  extractFileSize(sizeText) {
    ...
    if (extSizeText) {
      ...
      for (let i = sizeUnit.length - 1; i >= 0; i--) {
        if (
          new RegExp(`${sizeUnit[i]}|${sizeUnitSpare[i]}`, "i").test(sizeText)
        ) {
          index = i;
          break;
        }
